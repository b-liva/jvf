...
import {mutateTest} from "../../../graphql/cost/mutation/cost/test.graphql";
...
const {mutate: createTest, loading, error, onDone} = useMutation(mutateTest,
    () => ({
      variables: {
        'name': name.value,
      }
...
<div class="p-6 rounded bg-gray-100">
  ...
  <div class="mb-2">
    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">نام تست</label>
    <input
        v-model="name"
