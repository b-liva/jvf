...
import {mutateCertificate} from "../../../graphql/cost/mutation/cost/certificate.graphql";
...
const {mutate: createTest, loading, error, onDone} = useMutation(mutateCertificate,
    () => ({
      variables: {
        'name': name.value,
      }
...
<div class="p-6 rounded bg-gray-100">
  ...
  <div class="mb-2">
    <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">نام گواهی</label>
    <input
        v-model="name"
  ...
  <div class="w-full text-left">
    <button class="bg-blue-500 text-white p-1.5 rounded" @click="createTest">ذخیره</button>
  </div>
</div>
</template>
